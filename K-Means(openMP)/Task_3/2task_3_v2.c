...
static inline float EuclDist(float *V , float *C){
	...
	// #pragma omp parallel for schedule(static) reduction(+:dist) //EDO EPHREASMENOS APO THN ERGASIA 2 KAI AFOY EXW PARALLHLOPOIHSH TA LOOP STA OPOOIA KALEITAI
	for(j=0;j<Nv;j++){												//H EUCLDIST DEN THN PARALLHLOPOIW
		float dx = V[j] - C[j];
		dist += dx*dx;
