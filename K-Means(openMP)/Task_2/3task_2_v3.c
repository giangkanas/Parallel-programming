...
static inline float EuclDist(float *V , float *C){
	...
	// #pragma omp parallel for reduction(+:dist) //EDO DOKIMAZW NA MHN PARALLHLOPOIHSW THN EuclDist NA DW PVW THA EPHREASTEI O SYNOLIKOS XRONOS KAI FAINETAI VELTIWSH
	for(j=0;j<Nv;j++){							
		float dx = V[j] - C[j];                 
		dist += dx*dx;
