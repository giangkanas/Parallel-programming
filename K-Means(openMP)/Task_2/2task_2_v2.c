...
static inline float EuclDist(float *V , float *C){
	...
	#pragma omp parallel for reduction(+:dist) //EDO DOKIMAZW NA PARALLHLOPOIHSW THN EuclDist GIATI APO TO REPORT FANHKE OTI STO 99% BRISKETAI MESA EDW
	for(j=0;j<Nv;j++){							
		float dx = V[j] - C[j];
		dist += dx*dx;
...
void Classification(void){
	...
	#pragma omp parallel for reduction(+:totalDist)//EDW PERA APO THN EUCLDIST PROSPATHW NA PARALLHLOPOIHSW THN SYNARTHSH MESA STHN OPOIA KALEITAI H EuclDist GIA 
	for(int i=0;i<N;i++){						//NA DW AN YPARKSEI VELTIWSH KAI ONTWS YPARXEI VELTIWSH
