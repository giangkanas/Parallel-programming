...
static inline float EuclDist(float *V , float *C){
	...
	// #pragma omp parallel for reduction(+:dist) //EFOSON EXW PARALLHLOPOIHSEI TO LOOP STO OPOIO KALEITAI H EUCLDIST MESA STHN CLASSIFICATION 
	for(j=0;j<Nv;j++){							//DOKIMAZW NA MHN PARALLHLOPOIHSW THN EuclDist PRAGMA POY OMWS DEN ME ODHGEI SE KALYTERO APOTELESMA APO AYTO THS VERSION3
		float dx = V[j] - C[j];                 
		dist += dx*dx;
...
void Classification(void){
	...
	for(int i=0;i<N;i++){						//KAI ONTWS YPARXEI VELTIWSH
		float minDist = INFINITY;
		float *vector = Vec[i];
		
		#pragma omp parallel for //EDW PARALLHLOPOIW KAI DEYTERO LOOP MESA STHN CLASSIFICATION STO OPOIO KALEITAI H EUCLDIST KAI VLEPW VELTIWSH
		for(int k=0;k<Nc;k++){
...
void EstimateCenters(void){ //APO TO REPORT THS SEIRIAKHS EKDOSHS PROEKYPSSE OTI H ESTIMATECENTERS EINAI TRITH SE POSOSTO OPOTE DOKIMAZV NA THN PARALLHLOPOIHSV MAZI 
							//ME THN CLASSIFICATION POY MEXRI STIGMHS MOY EXEI DVSEI TO KALYTERO APOTELESMA. APO TO APOTELESMA PROEKYPSE OTI DEN BELTIWTHIKE H TAXYTHTA
	int i,j,k;
	int count=0;
