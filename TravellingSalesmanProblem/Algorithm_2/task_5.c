...
float totalDistance(){
	...
	for(index_counter=0; index_counter<C-1; index_counter++){
        minDist[0] = INFINITY;
        minDist[1] = INFINITY;
        minIndex[0] = 0;
        minIndex[1] = 0;

        current[0] = Cities[path[index_counter]][0]; // OYSIASTIKA EDW PROSPATHW NA APEKSARTISW KATHE ITERATION APO TO PROHGOYMENO
        current[1] = Cities[path[index_counter]][1];

        #pragma omp parallel for private(next,dist) shared(current,minDist,minIndex)
        for(j=0;j<C-index_counter-1;j++){
            ...
            // printf("\t\tARR[%d] = %d\tDIST(%f,%f) = %f\tTHREAD_NO = %d\n",j,arr[j],current[0],next[0],dist,omp_get_thread_num());
            #pragma omp critical
            {if(dist<minDist[0]){
                minDist[1] = minDist[0];
            ...
            else if(dist<minDist[1] && first==0){
                minDist[1] = dist;
                minIndex[1] = j;
            }
            first = 0;}
        ...
        // printf("\t\tMINDIST = %f\tMININDEX = %d\tTHREAD_NO = %d\n",min_dist,arr[min_index],omp_get_thread_num());


        deleteElement(min_index,index_counter+1);
...
void deleteElement(int min,int counter){  //AYTH H SYNARTHSH DIAGRAFEI THN POLH APO TO ARRAY arr
    ...
    path[counter] = arr[min]; //FTIAXNW ENA PINAKA ME TO PATH
